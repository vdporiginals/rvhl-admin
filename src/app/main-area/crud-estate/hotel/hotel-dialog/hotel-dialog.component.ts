...
export class HotelDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.data.id) {
      ...
      this.api.getData(this.data.id, 'estates/hotel').subscribe(res => {
        this.dataEdit = res;
        this.isEdit = true;
        ...
        this.detailForm.get('facilities.meetingRoom').setValue(res.data.facilities.meetingRoom);
  ...
  createOrUpdate(val?) {
    ...
    if (this.data.id) {
      this.api.updateData(this.detailForm.value, this.data.id, 'estates/hotel').subscribe(() => { }, (err: any) => {
        this.noti.showError('Sửa Hotel thất bại', err);
      }, () => {
        this.noti.showSuccess('Sửa Hotel Thành công', '');
        this.dialogRef.close();
      });
    } else {
      this.api.postData(this.detailForm.value, 'estates/hotel').subscribe(() => { }, (err: any) => {
        this.noti.showError('Tạo Hotel thất bại', err.error);
      }, () => {
        this.noti.showSuccess('Tạo Hotel Thành công', '');
        this.dialogRef.close();
      });
    }
  }
