...
export class HotelComponent implements OnInit, AfterViewInit {
  ...
  update(id) {
    ...
    this.api.getDataWithPosition('estates/category', 1, 10, 'name,position', '', 'Hotel').pipe().subscribe(res => {
      dialogConfig.data = {
        id,
        category: res
      };
      this.dialog.open(HotelDialogComponent, dialogConfig).afterClosed()
        .subscribe(() => this.loadDatasPage());
    });
  }

  create() {
    this.api.getDataWithPosition('estates/category', 1, 10, 'name,position', '', 'Hotel').pipe().subscribe((res: any) => {
      this.dialog.open(HotelDialogComponent, res).afterClosed()
        .subscribe(() => this.loadDatasPage());
    });
  }
