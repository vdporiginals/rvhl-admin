...
import { HomestayDialogComponent } from './homestay-dialog/homestay-dialog.component';
...
export class HomestayComponent implements OnInit, AfterViewInit {
  ...
  dataSource: any;
  count: number;
  private apiName = 'estates/homestay';
  ...
  update(id) {
    ...
    this.api.getDataWithPosition('estates/category', 1, 10, 'name,position', '', 'Homestay').pipe().subscribe(res => {
      dialogConfig.data = {
        id,
        category: res
      };
      this.dialog.open(HomestayDialogComponent, dialogConfig).afterClosed()
        .subscribe(() => this.loadDatasPage());
    });
  }

  create() {
    this.api.getDataWithPosition('estates/category', 1, 10, 'name,position', '', 'Homestay').pipe().subscribe((res: any) => {
      this.dialog.open(HomestayDialogComponent, res).afterClosed()
        .subscribe(() => this.loadDatasPage());
    });
  }
